trigger Product2AdallomTrigger on Product2 (after delete, after insert, after update) { 
    try{
        AdallomAudit__c audit = new AdallomAudit__c();
        audit.UserId__c = UserInfo.getUserId();
        audit.Timestamp__c = System.now(); 
        audit.ObjectName__c = 'Product2';
        if (Trigger.isDelete) {
            if (Trigger.old.size() == 1) {
                ...
                audit.ObjectTitle__c = Trigger.old[0].Name;
            } else {
            ...
            if (Trigger.isInsert) {
                if (Trigger.new.size() == 1) {
                    ...
                    audit.ObjectTitle__c = Trigger.new[0].Name;
                } else {
